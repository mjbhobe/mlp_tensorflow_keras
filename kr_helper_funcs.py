...
def progbar_msg(curr_tick, max_tick, head_msg, tail_msg, final=False):
    ...
    if not final:
        ...
        prog_msg = '  %s (%*d/%*d) [%s] %s%s\n' % (
            head_msg, len_max_tick, max_tick, len_max_tick, max_tick, '=' * progbar_len, 
            tail_msg, ' '*35)
        print('\r%s' % prog_msg, end='', flush=True)
...
def show_plots(history):
    ...
    if val_loss_vals is not None: del val_loss_vals
    if val_acc_vals is not None: del val_acc_vals
...
def save_keras_model(model, base_file_name, save_dir='.'):
...
def load_keras_model(base_file_name, load_dir='.'):
